...
class Singleton_SynchronizedDoubleLockingTest {

    public static final int THREAD_COUNT = 10000;

    @org.junit.jupiter.api.Test
    public void testingThreadSafety(){
        final Singleton_Synchronized_DoubleLocking[] singleton = new Singleton_Synchronized_DoubleLocking[THREAD_COUNT];
        final AtomicInteger count = new AtomicInteger(THREAD_COUNT);
        for(int i=0; i < THREAD_COUNT; i++){
            final int l = i;
            new Thread() {
                public void run() {
                    singleton[l] = Singleton_Synchronized_DoubleLocking.getInstanceUsingDoubleLocking();
